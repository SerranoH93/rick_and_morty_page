...
describe("Test de RUTAS", () => {
    ...
    describe("GET /rickandmorty/character/:id", () => {
        ...
        it("Si hay un error responde con status: 500", async () => {
            await agent.get('/rickandmorty/character/123456789').expect(404);//no se por que es 404
        })
    })

    describe("GET /rickandmorty/login", () => {
        it("Retorna objeto { access: true } con credenciales correctas", async () => {
            const response = await agent.get("/rickandmorty/login?email=alejandro_0793@hotmail.com&password=123456");
